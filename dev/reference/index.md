# Package index

## BehaviorSpace

Functions to set up and run NetLogo experiments using
[BehaviorSpace](https://docs.netlogo.org/behaviorspace.html).

- [`create_experiment()`](https://danielvartan.github.io/logolink/dev/reference/create_experiment.md)
  : Create NetLogo BehaviorSpace experiment
- [`inspect_experiment()`](https://danielvartan.github.io/logolink/dev/reference/inspect_experiment.md)
  : Inspect NetLogo BehaviorSpace experiment file
- [`run_experiment()`](https://danielvartan.github.io/logolink/dev/reference/run_experiment.md)
  : Run NetLogo BehaviorSpace experiment
- [`read_experiment()`](https://danielvartan.github.io/logolink/dev/reference/read_experiment.md)
  : Read NetLogo BehaviorSpace Experiment output

## Parsers

Functions to parse NetLogo data types.

- [`parse_netlogo_color()`](https://danielvartan.github.io/logolink/dev/reference/parse_netlogo_color.md)
  : Parse NetLogo colors
- [`parse_netlogo_list()`](https://danielvartan.github.io/logolink/dev/reference/parse_netlogo_list.md)
  : Parse NetLogo lists

## System

Functions to manage NetLogo installations.

- [`find_netlogo_home()`](https://danielvartan.github.io/logolink/dev/reference/find_netlogo_home.md)
  : Find NetLogo installation directory
- [`find_netlogo_console()`](https://danielvartan.github.io/logolink/dev/reference/find_netlogo_console.md)
  : Find NetLogo executable file
- [`find_netlogo_version()`](https://danielvartan.github.io/logolink/dev/reference/find_netlogo_version.md)
  : Find NetLogo version

## Utilities

Utility functions for NetLogo integration.

- [`get_netlogo_shape()`](https://danielvartan.github.io/logolink/dev/reference/get_netlogo_shape.md)
  : Download NetLogo shapes from LogoShapes
