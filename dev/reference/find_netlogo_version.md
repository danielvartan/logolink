...
1.  If the NetLogo console executable is found by
    ...
    it runs `NetLogo_Console --headless --version` command to retrieve
    the version information. This is the most reliable method.

2.  If the executable is not found, it attempts to extract the version
    number from the installation directory name returned by
