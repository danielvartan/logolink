...
Next, we need to locate the model. We'll use the [`find_netlogo_home()`](https://danielvartan.github.io/logolink/reference/find_netlogo_home.html) function to find your NetLogo installation, then navigate to the model file:
...
Here's where things get interesting. We want to capture the position of every sheep, wolf, and patch at regular intervals. Let's set up an experiment with [`create_experiment()`](https://danielvartan.github.io/logolink/reference/create_experiment.html) that takes these snapshots every 100 ticks:
...
Now let's run it using [`run_experiment`](https://danielvartan.github.io/logolink/reference/run_experiment.html):
...
NetLogo uses its own color coding system, so we need to convert those values to [hex colors](https://en.wikipedia.org/wiki/Web_colors) that [`ggplot2`](https://ggplot2.tidyverse.org/) understands. We'll use the [`dplyr`](https://dplyr.tidyverse.org/) package to mutate the relevant columns with the [`parse_netlogo_color`](https://danielvartan.github.io/logolink/reference/parse_netlogo_color.html) function:
...
Static plots are nice, but an animation brings the simulation to life. Let's use the [`magick`](https://docs.ropensci.org/magick/) package to stitch our snapshots together.
