...
experiment. We can do this using the `create_experiment` function. This
function allows us to define the parameters we want to vary, the metrics
we want to collect, and other settings for our simulation runs.
...
`create_experiment` step and simply pass the experiment name to
`run_experiment`.
...
`run_experiment` function. This function takes care of launching
NetLogo, executing the experiment, and collecting the results as [tidy
data frames](https://r4ds.hadley.nz/data-tidy.html).
...
We can now examine the results. The output from `run_experiment` is a
list of data frames. These can be any of the [four output
...
tutorial to learn how to do this by using `parse_netlogo_color` and
`get_netlogo_shape` functions.
