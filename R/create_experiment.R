...
#'
#' Note that [`character`][base::character()] strings should be passed as is,
#' without adding quotes to them. For example, to set the variable
#' `scenario` to `"SSP-585"`, you should use `list("scenario" = "SSP-585")`,
#' not `list("scenario" = '"SSP-585"')`.
#'
...
#'   specifying the NetLogo command to run after each run (default: `NULL`).
...
#'   (for fixed/enumerated values) or a [`list`][base::list()] with `first`,
#'   `step`, and `last` elements (for stepped/varying values). See the *Details*
#'   and *Examples* sections to learn more (default: `NULL`).
